...
DimPlot(cancer6, reduction = "pca")+
  FeaturePlot(cancer6,features = "dtd_FLD004", reduction = "pca")
DimPlot(cancer6,reduction = "pca")+
  FeaturePlot(cancer6,
...
cancer6 <- RenameIdents(cancer6, `2` = "MCF10A", `1` = "MCF7", `0` = "MCF7")
...
cancer_merge123456 <- RunTSNE(cancer_merge123456,dims=1:10)
  ...
  FeaturePlot(subset(cancer_merge123456,subset=percent.mt<10),
  ...
  p1<-FeaturePlot(subset(subset(cancer_merge123456,subset=NEP== "0V"),subset=condition=="normal"),
    ...
    p3<-DimPlot(subset(subset(cancer_merge123456,subset=percent.mt<10),subset=condition=="normal"),reduction = "tsne",group.by = "celltype")
    p4<-DimPlot(subset(cancer_merge123456,subset=condition=="normal"),reduction = "tsne",group.by = "NEP")
    p5<-DimPlot(subset(subset(cancer_merge123456,subset=percent.mt<10),subset=condition=="normal"),reduction = "tsne",group.by = "NEP")
    p6<-DimPlot(subset(subset(cancer_merge123456,subset=percent.mt<5),subset=condition=="normal"),reduction = "tsne",group.by = "NEP")
...
DimPlot(subset(cancer_merge123456,subset=percent.mt<5),reduction = "umap",group.by = "celltype")+
  FeaturePlot(subset(cancer_merge123456,subset=percent.mt<5),
              features = c("CYBA","S100A2","CCDC190","CAVIN3","CD163L1","PLAU","FOSL1"),
              reduction = "umap",
              max.cutoff = 4)

rm(cancer,cancer_mcf10,cancer_mcf7,cancer_mda,cancer_merge,cancer_merge12345,cancer_pc3,cancer_pc3_CTL,cancer_pc3_NEP)
rm(cancer1,cancer2,cancer2_mcf7,cancer2_pc3,cancer3_merge,cancer3,cancer6,cancer6_merge)
