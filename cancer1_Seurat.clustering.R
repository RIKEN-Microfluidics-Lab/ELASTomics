...
#subset(cancer,subset=run=="first") <- RenameIdents(subset(cancer,subset=run=="first"), `0` = "MCF7", `1` = "PC3")

# FeatureScatter(subset(cancer,subset=run=="first"),feature2 = "percent.mt",feature1 = "nCount_RNA",pt.size = 0.1)+
...
#cancer1<-merge(cancer1,y=cancer_pc3_CTL)
#cancer1<-subset(cancer,subset=run=="first")
#cancer2<-subset(cancer,subset=run=="second")
...
VlnPlot(cancer_merge,features=c("dtd_FLD004","dtd_FLD500"),group.by = "celltype_NEP_condition")
