...
cancer<-NormalizeData(cancer,assay = "DTD",normalization.method = "CLR")
cluster = as.numeric(Idents(cancer))
cancer <- RunUMAP(cancer, dims = 1:10)
DimPlot(cancer,reduction="pca")+
  FeaturePlot(cancer,features = c("dtd_P3CE36","CYBA","S100A2","dtd_FLD004"),
              reduction = "pca",
              max.cutoff = 100)


cancer <- RenameIdents(cancer, `0` = "PC3_NEP", `1` = "MCF7_CTL")
