...
exp.matrix <- exp.matrix[,colnames(exp.matrix) %in% cors.sig[!(cors.sig$gene %in% "RRAD"),]$gene]
...
for (i in 1:length(alpha)) {
  m <- cv.glmnet(x = exp.matrix, y = response$RRAD, family = "gaussian", alpha = alpha[i])
  mse.df <- rbind(mse.df, data.frame(alpha = alpha[i],mse = min(m$cvm)))
}

best.alpha <- mse.df$alpha[mse.df$mse == min(mse.df$mse)]

m <- cv.glmnet(x = exp.matrix, y = response$RRAD, family = "gaussian", alpha = best.alpha)

best.lambda <- m$lambda.min

en.model <- glmnet(x = exp.matrix, y = response$RRAD, family = "gaussian",
...
ggplot(en.model.beta,aes(x=cors,y=s1))+geom_point()+
  geom_point(data=subset(en.model.beta,gene %in% c("AC007952.4","AC091271.1","ABL2","FOS","RRAD","RHOB","RPL22L1","YWHAH")),aes(y=s1,x=cors,color="red"))+
  geom_text_repel(data=subset(en.model.beta,gene %in% c("AC007952.4","AC091271.1","ABL2","FOS","RRAD","RHOB","RPL22L1","YWHAH")),aes(y=s1,x=cors,label=gene))+
  theme_bw()
